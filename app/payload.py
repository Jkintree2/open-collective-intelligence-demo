...
# A name we are about to create must be at least this long (05_extraction.md). A shorter name
# that already exists in the record is reused, because make_key() accepts two characters.
NEW_NAME_MIN = 3
MAX_ISSUES = 3
MAX_SOLUTIONS = 5
MAX_EVIDENCE = 5
...
class IssueItem(BaseModel):
    ...
    @classmethod
    def recompute_existing(cls, value: Any) -> bool:
...
class EvidenceItem(BaseModel):
    ...
    def _url(cls, value: Any) -> str | None:
        ...
        try:
            ...
            return value if valid and not any(char.isspace() for char in value) else None
        except ValueError:
            return None
...
class CardPayload(BaseModel):
    ...
    def clear_unfound(self) -> "CardPayload":
        if not self.language_ok or not self.found:
            self.issues, self.solutions, self.evidence = [], [], []
        return self
...
class Candidates:
    ...
    solution_issues: dict[str, list[str]] = field(default_factory=dict)
...
class ResolvedPayload:
    ...
    # Rows the rules repaired rather than lost. These are logged and shown on the card, but
    # they never block posting: 03_schema.md corrects a bad reference, it does not refuse it.
...
def _keyed(name: str | None) -> tuple[str | None, str]:
    """The key and display form of a name. References resolve on the key alone."""
    cleaned = clean_name(name or "")
    return make_key(cleaned), cleaned


def _usable(key: str | None, name: str, known: dict[str, Any]) -> bool:
    """A key we can write: either it already exists, or the name is long enough to create."""
    return key is not None and (key in known or len(name) >= NEW_NAME_MIN)
...
def resolve_payload(payload: CardPayload, candidates: Candidates) -> ResolvedPayload:
    ...
    for item in payload.issues:
        ...
        if not _usable(key, name, candidates.issues):
    ...
    for item in payload.solutions:
        key, name = _keyed(item.name)
        if not _usable(key, name, candidates.solutions):
    ...
    for item in payload.evidence:
        key, name = _keyed(item.name)
        if not _usable(key, name, candidates.evidence):
...
def _resolve_parent(
    ...
    current = candidates.issues.get(row["key"], {})
    if current.get("parent_key"):
        return current["parent_key"]
    if any(item.get("parent_key") == row["key"] for item in candidates.issues.values()):
        return None
    ...
    if known is None:
        sibling = issue_keys.get(parent_key)
        # A parent introduced by the same card counts, as long as it is top level itself.
        if sibling is not None and _keyed(sibling["parent_requested"])[0] is None:
            return parent_key
        out.corrected.append(f"parent {row['parent_requested']!r} of {row['name']!r} does not exist")
        return None
    if known.get("parent_key"):
        # A sub-issue cannot be a parent: use its own parent instead (M2).
        return known["parent_key"] if known["parent_key"] != row["key"] else None
    return parent_key
